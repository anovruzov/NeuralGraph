...
# Scratch directory for run files that are not committed (judge cache, rescored
# output, progress log) and for any result file kept outside the repo.
# Override with N5_SCRATCH_DIR; defaults to research/results/scratch/.
OV = os.environ.get('N5_SCRATCH_DIR', str(Path(__file__).resolve().parents[1] / 'results' / 'scratch')).rstrip('/') + '/'
os.makedirs(OV, exist_ok=True)
DR = str(Path(__file__).resolve().parents[1] / 'results') + '/'
FILES = [
...
# Verbatim copy of research/benchmarks/runner.py ACCURACY_PROMPT (campaign judge prompt).
ACCURACY_PROMPT = """
