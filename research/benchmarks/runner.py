...
OUTPUT_PATH = Path(__file__).resolve().parents[2] / "research" / "results" / f"{RUN_NAME}.json"
...
async def run_benchmark():
    # Load LoCoMo data
    locomo_path = Path(__file__).resolve().parents[2] / "research" / "datasets" / "locomo10.json"
        ...
        for conv_idx, conv in enumerate(data):
            ...
            if RETRIEVAL_MODE in ("local_pairs", "mega"):
                ...
                if RETRIEVAL_MODE == "mega":
                    from NeuralGraph.research.retrieval.mega_search import MegaIndex
                    kg = None
                    if os.environ.get("KG", "0") == "1":   # LLM-built entity graph from NeuralGraph.research.retrieval.kg_extractor
                        kg_path = Path(__file__).resolve().parents[2] / "research" / "results" / "kg_cache" / f"conv{conv_idx}.json"
