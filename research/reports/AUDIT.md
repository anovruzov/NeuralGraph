...
> **Status note added 2026-09-22.** This audit is kept verbatim as the record of what the
> repository looked like during the campaign. Since it was written: the integrity problems
> in section "Integrity problems" A–C were fixed (the gold-answer gate and gold-category
> routing were removed before any published run); D's credential was removed from the
> working tree and **still needs revoking at platform.openai.com**, as it remains in git
> history. The leaky December baseline it references (`maximal.json`) is no longer
> published in this repository. The stale root `__init__.py` was deleted, so `pytest`
> collects normally now. Paths below refer to the pre-reorganisation layout — see
> [the mapping](../README.md#reading-the-reports).
