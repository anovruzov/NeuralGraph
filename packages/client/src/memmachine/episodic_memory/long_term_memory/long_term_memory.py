...
class LongTermMemoryParams(BaseModel):
    ...
    Attributes:
        ...
        domain_classifier (DomainClassifier | None):
            Optional domain classifier for memory sharding.
    ...
    domain_classifier: InstanceOf[DomainClassifier] | None = Field(
        default=None,
        description="Optional domain classifier for memory sharding",
    )
...
class LongTermMemory:
    ...
    async def add_episodes(self, episodes: Iterable[Episode]) -> None:
        ...
        # Classify domains for each episode if classifier is available
        domain_values: list[str] = []
        if self._domain_classifier:
            for episode in episodes_list:
                classification = self._domain_classifier.classify_content(
                    episode.content
                )
                # Store primary domain for efficient filtering
                domain_values.append(classification.primary_domain.value)
        else:
            domain_values = [Domain.GENERAL.value] * len(episodes_list)
        ...
        declarative_memory_episodes = [
                ...
                filterable_properties=cast(
                    dict[str, FilterablePropertyValue],
                    {
                        ...
                        for key, value in {
                            ...
                            LongTermMemory._DOMAIN_PROPERTY_KEY: domain_value,
                        }.items()
            ...
            for episode, domain_value in zip(episodes_list, domain_values, strict=True)
        ]
    ...
    async def search(
        ...
        *,
        ...
        # Classify query to determine which domains to search
        effective_domains = domains
        if effective_domains is None and self._domain_classifier:
            classification = self._domain_classifier.classify_query(query)
            effective_domains = classification.domains

        # Build domain filter if domains are specified
        combined_filter = LongTermMemory._sanitize_property_filter(property_filter)
        if effective_domains:
            domain_filter = LongTermMemory._build_domain_filter(effective_domains)
            if combined_filter is not None:
                combined_filter = FilterAnd(left=combined_filter, right=domain_filter)
            else:
        ...
        declarative_memory_episodes = await self._declarative_memory.search(
            ...
            property_filter=combined_filter,
        )
        return [
    ...
    @staticmethod
    def _build_domain_filter(domains: list[Domain]) -> FilterExpr:
        """Build a filter expression to match any of the specified domains."""
        if len(domains) == 1:
            # Single domain: exact match
            return FilterComparison(
                field=LongTermMemory._DOMAIN_PROPERTY_KEY,
                op="=",
                value=domains[0].value,
            )

        # Multiple domains: use IN operator
        return FilterComparison(
            field=LongTermMemory._DOMAIN_PROPERTY_KEY,
            op="in",
            value=[domain.value for domain in domains],
        )
