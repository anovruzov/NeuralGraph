...
class IngestionService:
    ...
    async def _apply_commands(
        ...
        *,
        ...
        # Separate ADD and DELETE commands
        add_commands = [c for c in commands if c.command == SemanticCommandType.ADD]
        delete_commands = [c for c in commands if c.command == SemanticCommandType.DELETE]
        other_commands = [
            c for c in commands
            if c.command not in (SemanticCommandType.ADD, SemanticCommandType.DELETE)
        ]

        # Batch embed all ADD command values in a single call
        embedding_map: dict[str, list[float]] = {}
        if add_commands:
            values_to_embed = [c.value for c in add_commands]
            embeddings = await embedder.ingest_embed(values_to_embed)
            embedding_map = dict(zip(values_to_embed, embeddings))

        # Process ADD commands with pre-computed embeddings
        for command in add_commands:
            value_embedding = embedding_map[command.value]
    ...
    async def _deduplicate_features(
        ...
        *,
        ...
        # Batch embed all consolidated feature values in a single call
        consolidated_features = consolidate_resp.consolidated_memories
        if consolidated_features:
            values_to_embed = [f.value for f in consolidated_features]
            embeddings = await resources.embedder.ingest_embed(values_to_embed)
            embedding_map = dict(zip(values_to_embed, embeddings))

            # Add all features with pre-computed embeddings
            for f in consolidated_features:
                value_embedding = embedding_map[f.value]
