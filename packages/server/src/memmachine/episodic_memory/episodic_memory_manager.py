...
class EpisodicMemoryManager:
    ...
    def __init__(self, params: EpisodicMemoryManagerParams) -> None:
        ...
        self._check_instance_task: asyncio.Task | None = asyncio.create_task(
            self._check_instance_life_time(),
        )

    async def _check_instance_life_time(self) -> None:
        """Check instance lifetime with proper shutdown handling."""
        while not self._shutdown_event.is_set():
            try:
                # Wait for shutdown event with timeout (allows faster shutdown)
                ...
                # Normal timeout - do the cleanup check
    ...
    async def close(self) -> None:
        ...
        if self._check_instance_task is not None:
            self._check_instance_task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await self._check_instance_task
            self._check_instance_task = None
