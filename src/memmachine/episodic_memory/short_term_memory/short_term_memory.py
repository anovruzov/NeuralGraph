...
from memmachine.common.episode_store import Episode, EpisodeStorage
...
class ShortTermMemoryParams(BaseModel):
    ...
    Attributes:
        ...
        episode_storage (EpisodeStorage): Storage for persisting episodes.
    ...
    episode_storage: InstanceOf[EpisodeStorage] | None = Field(
        default=None,
        description="Episode storage for persisting and retrieving episodes",
    )
...
class ShortTermMemory:
    ...
    def __init__(
        ...
        self._episode_storage: EpisodeStorage | None = param.episode_storage
    ...
    async def create(cls, params: ShortTermMemoryParams) -> "ShortTermMemory":
        ...
        if params.data_manager is not None:
            ...
            try:
                ...
                # Retrieve episodes from storage if available
                episodes: list[Episode] = []
                if params.episode_storage is not None and episode_num > 0:
                    try:
                        # Query episodes for this session using filter expression
                        session_filter = Comparison(
                            field="session_key",
                            op="=",
                            value=params.session_key,
                        )
                        all_episodes = await params.episode_storage.get_episode_messages(
                            filter_expr=session_filter,
                            page_size=episode_num,
                        )
                        # Sort by sequence number and take the most recent episodes
                        all_episodes.sort(key=lambda e: e.sequence_num, reverse=True)
                        episodes = list(reversed(all_episodes[:episode_num]))
                        logger.debug(
                            "Restored %d episodes for session %s",
                            len(episodes),
                            params.session_key,
                        )
                    except Exception as e:
                        logger.warning(
                            "Failed to retrieve episodes from storage: %s",
                            e,
                        )

                return ShortTermMemory(params, summary, episodes if episodes else None)
            except ValueError:
                pass
