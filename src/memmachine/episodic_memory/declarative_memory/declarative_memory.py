...
class DeclarativeMemory:
    ...
    async def search(
        ...
        *,
        ...
        # Apply recency boost to favor more recent memories
        episode_context_scores = DeclarativeMemory._apply_recency_boost(
            scores=episode_context_scores,
            nuclear_episodes=nuclear_episodes,
        )
    ...
    def _apply_recency_boost(
        ...
        Args:
            scores: Base scores from reranker.
            nuclear_episodes: Episodes corresponding to each score.
            decay_factor: Decay rate per day (~1% per day default).
            recency_weight: Weight of recency factor (0.3 = 30% recency, 70% base).
            reference_time: Reference time for age calculation (default: now UTC).

        Returns:
            Boosted scores with recency factor applied.
        """
        if reference_time is None:
            reference_time = datetime.datetime.now(tz=timezone.utc)

        boosted_scores = []
        for score, episode in zip(scores, nuclear_episodes, strict=True):
            # Calculate age in days
            episode_time = episode.timestamp
            if episode_time.tzinfo is None:
                episode_time = episode_time.replace(tzinfo=timezone.utc)

            age_seconds = (reference_time - episode_time).total_seconds()
            age_days = max(0, age_seconds / 86400)

            # Apply exponential decay
            recency_factor = math.exp(-decay_factor * age_days)

            # Blend base score with recency factor
            # (1 - recency_weight) of base score + recency_weight of recency-adjusted score
            boosted_score = score * ((1 - recency_weight) + recency_weight * recency_factor)
