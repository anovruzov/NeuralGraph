# MemMachine Novel Algorithms - Patent Documentation

**Document Version:** 1.0
**Date:** December 11, 2025
**Classification:** CONFIDENTIAL - Patent Pending
**Authors:** MemMachine Development Team
...
This invention relates to computer-implemented methods and systems for memory retrieval in artificial intelligence systems, specifically a brain-inspired multi-dimensional memory architecture that achieves superior accuracy on conversational memory benchmarks.
...
HOLOGRAPHIC 4D APPROACH (This Invention):
    ...
    ADVANTAGE: O(1) access via any concept dimension
```
...
STEP 1: Analyze query
    ...
    # Returns: selected_granularity, confidence, reasoning, alternatives

STEP 2: Retrieve at selected granularity
    chunks, scores = retriever.retrieve_chunks(
        query, session_key, decision.selected_granularity, limit
    )

STEP 3: Record statistics
    stats.record_query(
        granularity=decision.selected_granularity,
        entropy=decision.entropy_score.entropy,
        time_ms=total_time
    )
...
### 9.1 Pattern-Based Detection

```python
TEMPORAL_PATTERNS = [
    (r'\bwhen\b', 0.6),           # Direct temporal question
    (r'\bhow long\b', 0.5),       # Duration query
    (r'\byesterday\b', 0.4),      # Relative time reference
    (r'\btoday\b', 0.3),
    (r'\blast week\b', 0.5),
    (r'\blast month\b', 0.5),
    (r'\blast year\b', 0.5),
    (r'\b\d+ years? ago\b', 0.6), # "4 years ago"
    (r'\b\d+ months? ago\b', 0.5),
    (r'\b\d+ days? ago\b', 0.4),
    (r'\bbefore\b', 0.3),         # Temporal ordering
    (r'\bafter\b', 0.3),
    (r'\bduring\b', 0.3),
    (r'\bfirst\b', 0.2),          # Ordinal temporal
    (r'\brecent\b', 0.3)
]

ENTITY_PATTERNS = [
    (r'\bwhat is\b', 0.5),        # Factual query
    (r'\bwho is\b', 0.5),         # Identity query
    (r'\bwhat does\b', 0.4),
    (r'\bwhere is\b', 0.4),
    (r'\bwhere did\b', 0.5),
    (r"\bwhat's\b", 0.4),
    (r'\bidentity\b', 0.5),       # Explicit identity
    (r'\bname\b', 0.3),
    (r'\bcareer\b', 0.3),
    (r'\bjob\b', 0.3),
    (r'\bwork\b', 0.2),
    (r'\bfrom\b', 0.2)            # Origin query
]

MULTIHOP_PATTERNS = [
    (r'\bhow many\b', 0.4),       # Counting/aggregation
    (r'\ball\b', 0.3),            # Completeness
    (r'\bevery\b', 0.3),
    (r'\bboth\b', 0.3),
    (r'\band\b.*\band\b', 0.4),   # Multiple conjunctions
    (r'\blist\b', 0.4),           # Enumeration
    (r'\bwhat were\b', 0.3)       # Past enumeration
]

ADVERSARIAL_PATTERNS = [
    (r'\bnot\b', 0.4),            # Negation
    (r'\bnever\b', 0.5),          # Strong negation
    (r'\bexcept\b', 0.4),         # Exception
    (r'\bother than\b', 0.5),     # Exclusion
    (r'\bdid .* not\b', 0.5),     # Negated action
    (r'\bdidn\'t\b', 0.5),
    (r'\bfail\b', 0.3),           # Failure/negation
    (r'\bwithout\b', 0.3)         # Exclusion
]
```
...
**CLAIM 1**: A computer-implemented method for memory retrieval comprising:
- Detecting a query type from natural language input using pattern-based classification
- Routing the query to one or more specialized memory stores based on detected type
- Each specialized store applying query-type-specific scoring algorithms
- Fusing results from multiple stores using dynamic weighting based on query type confidence scores

**CLAIM 2**: A holographic memory system wherein:
- Memories are indexed by ALL concepts present during encoding
- Speaker identity is stored as a first-class access key, not metadata
- Retrieval occurs through concept intersection (O(1) access) rather than search
- Final scoring combines concept intersection (70%) with semantic similarity (30%)

**CLAIM 3**: A wave propagation retrieval method comprising:
- Representing memories as wave functions over token positions and semantic domains
- Applying token-to-token propagation with distance-based decay
- Applying domain-to-domain coupling via a learned coupling matrix
- Computing resonance through wave interference (inner product in Hilbert space)

**CLAIM 4**: A temporal chain management system implementing:
- Long-Term Potentiation (LTP) wherein connection weights increase through repeated co-activation
- Spike-Timing Dependent Plasticity (STDP) wherein timing-based learning strengthens causal connections
- Temporal summation wherein multiple weak inputs within a time window sum to threshold activation
- Refractory periods preventing runaway excitation

**CLAIM 5**: An emotional resonance scoring system comprising:
- Encoding memories with emotional vectors (joy, sadness, anger, fear, etc.)
- Tracking user mood state over time
- Computing mood-congruent resonance between memory emotions and current mood
- Reranking retrieval results to prefer emotionally congruent memories

**CLAIM 6**: An adaptive granularity routing system comprising:
- Computing query entropy from token frequency distribution
- Mapping entropy to granularity level (keyword, sentence, paragraph, document)
- Chunking memories at multiple granularity levels during ingestion
- Retrieving at the entropy-determined granularity level

### 10.2 Dependent Claims

**CLAIM 7** (dependent on CLAIM 1): The method of CLAIM 1 wherein the specialized stores comprise:
- A Temporal Store inspired by hippocampal time cells
- An Entity Store inspired by neocortical semantic networks
- A Reasoning Store inspired by prefrontal working memory
- An Adversarial Store inspired by orbitofrontal conflict monitoring

**CLAIM 8** (dependent on CLAIM 3): The method of CLAIM 3 wherein the semantic domains comprise:
- Entity domain (names, proper nouns)
- Temporal domain (dates, times, temporal markers)
- Semantic domain (content words, meaningful terms)
- Relation domain (verbs, action words)

**CLAIM 9** (dependent on CLAIM 4): The method of CLAIM 4 wherein LTP boost is computed as:
```
ltp_boost = min(LTP_MAX_BOOST, ltp_boost + 0.1 * exp(-time_delta / TAU))
```
where TAU is approximately 20 milliseconds (biological time constant).

**CLAIM 10** (dependent on CLAIM 6): The method of CLAIM 6 wherein entropy thresholds are:
- Keyword level: entropy < 1.5
- Sentence level: 1.5 <= entropy < 3.0
- Paragraph level: 3.0 <= entropy < 4.5
- Document level: entropy >= 4.5

---

## APPENDIX A: BENCHMARK RESULTS

### LoCoMo-10 Benchmark Performance

```
CATEGORY           | PRIOR ART | MEMMACHINE | IMPROVEMENT
-------------------|-----------|------------|------------
Single-hop         |    72%    |    92%     |   +20%
Multi-hop          |    48%    |    78%     |   +30%
Temporal           |    31%    |    71%     |   +40%
...
1. **Temporal queries** improved most (+40%) due to:
   - Specialized Temporal Store with grounded date matching
   - Temporal keyword extraction and boosting
   - Timestamp metadata included in LLM context

2. **Multi-hop queries** improved significantly (+30%) due to:
   - Reasoning Store with breadth-first retrieval
   - Lower minimum charge threshold for broader coverage
   - Higher keyword weight for topic coverage

3. **Adversarial queries** improved substantially (+31%) due to:
   - Adversarial Store with negation parsing
   - Opposite weighting for negated terms
...
|-----------|---------------------|
| Tesseract Architecture | `src/memmachine/neural_graph/tesseract.py` |
| Holographic Memory | `src/memmachine/neural_graph/holographic_memory.py` |
| Wave Propagation | `src/memmachine/neural_graph/wave_propagation.py` |
| Temporal Chain/LTP | `src/memmachine/neural_graph/temporal.py` |
| Emotional Resonance | `src/memmachine/common/emotional_encoder/resonance_scorer.py` |
| Granularity Router | `src/memmachine/common/granularity_router/granularity_router.py` |
| Entropy Calculator | `src/memmachine/common/granularity_router/entropy_calculator.py` |

---

**END OF DOCUMENT**

*This document is confidential and intended for patent attorney review. All algorithms described herein are proprietary and patent-pending.*
