...
| B6_hier_no_lineage | Mycelic topology with replica counts as support: no signatures, no correlation discount, no cross-source consistency, no two-child rule; bare claims accepted by replica count |
| B7_mycelic | lineage-aware hierarchy: signed artifacts (forged provenance is dropped), correlation-discounted independent support, two-child synthesis rule, cross-source consistency conflicts, recent-window revision, k-anonymous sketch promotion |
